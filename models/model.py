...
class FCN8VGG:
    ...
    def bn_scale_relu_conv(self, bottom, train, nout, conv_type = 'conv', name='BatchNorm', ks=3, stride=1, pad='SAME'):
        ...
        with tf.variable_scope(name):
            ...
            axes = list(range(len(bottom.shape()) - 1))
            batch_mean, batch_var = tf.nn.moments(bottom, axes, name='moments')
            ...
            bn=tf.nn.batch_normalization(bottom, mean, var, beta, gamma, eps)

            relu=tf.nn.relu(bn)

            kernel_size=[ks,ks]
            return tf.cond(tf.equal(conv_type, 'conv'), lambda :layers.conv2d(relu,nout,kernel_size,stride,pad),
                           lambda :layers.conv2d_transpose(relu,nout,kernel_size,stride,pad))
    ...
    def conv_bn_scale_relu(self, bottom, train, nout, conv_type = 'conv', name='BatchNorm', ks=3, stride=1, pad='SAME'):
        ...
        with tf.variable_scope(name):
            ...
            beta = tf.get_variable('beat', param_shape, initializer=tf.constant_initializer(0))
            ema = tf.train.ExponentialMovingAverage(moving_decay)
            axes = list(range(len(conv.shape()) - 1))
    ...
    def build(self, batch_size, img_w=512, img_h=256, train=False, num_classes=19, city='Taipei', 
        ...
        with tf.variable_scope('feature_extractor'):
            ...
            data_f=self.bn_scale_relu_conv(self.bgr,train,nout=1,ks=ks)
            for i in range(repeat):
                result1=self.bn_scale_relu_conv(result,train,nout,ks=ks)
                result2=self.bn_scale_relu_conv(result1,train,nout,ks=ks)
                result=tf.add(result2,result)
            scale0=result
            scale0_m= self.bn_scale_relu_conv(scale0,train,nout=1,ks=ks)

            for i in range(repeat):
                if i==0:
                    result1=self.bn_scale_relu_conv(result,train,nout=2*nout,ks=ks,stride=2)
                    result=self.bn_scale_relu_conv(result,train,nout=2*nout,ks=1,stride=1)
                else:
                    result1=self.bn_scale_relu_conv(result,train,nout=2*nout,ks=ks)
                result2=self.bn_scale_relu_conv(result1,train,nout=2*nout,ks=ks)
            scale1=result
            scale1_m =self.bn_scale_relu_conv(scale1,train,nout=1,ks=ks)

            for i in range(repeat):
                if i==0:
                    result1=self.bn_scale_relu_conv(result,train,nout=nout*4,ks=ks,stride=2)
                    result=self.bn_scale_relu_conv(result,train,nout=4*nout,ks=1,stride=2)
                else:
                    result1=self.bn_scale_relu_conv(result,nout=nout*4,ks=ks,stride=1)
                result2=self.bn_scale_relu_conv(result1,train,nout=4*nout,ks=ks)
                result=tf.add(result2,result)
            scale2=result
            scale2_m=self.bn_scale_relu_conv(scale2,train,nout=1,ks=ks)

            for i in range(repeat):
                if i==0:
                    result1=self.bn_scale_relu_conv(result,train,nout=nout*8,ks=ks,stride=2)
                    result=self.bn_scale_relu_conv(result,train,nout=8*nout,ks=1,stride=2)
                else:
                    result1=self.bn_scale_relu_conv(result,nout=nout*8,ks=ks,stride=1)
                result2=self.bn_scale_relu_conv(result1,train,nout=8*nout,ks=ks)
                result=tf.add(result2,result)
            scale3=result
            scale3_m=self.bn_scale_relu_conv(scale3,train,nout=1,ks=ks)

            for i in range(repeat):
                if i==0:
                    result1=self.bn_scale_relu_conv(result,train,nout=nout*16,ks=ks,stride=2)
                    result=self.bn_scale_relu_conv(result,train,nout=16*nout,ks=1,stride=2)
                else:
                    result1=self.bn_scale_relu_conv(result,nout=nout*16,ks=ks,stride=1)
                result2=self.bn_scale_relu_conv(result1,train,nout=16*nout,ks=ks)
                result=tf.add(result2,result)
            scale4=result
            scale4_m=self.bn_scale_relu_conv(scale4,train,nout=1,ks=ks)

            for i in range(repeat):
                if i==0:
                    result1=self.bn_scale_relu_conv(result,train,nout=nout*32,ks=ks,stride=2)
                    result=self.bn_scale_relu_conv(result,train,nout=32*nout,ks=1,stride=2)
                else:
                    result1=self.bn_scale_relu_conv(result,nout=nout*32,ks=ks,stride=1)
                result2=self.bn_scale_relu_conv(result1,train,nout=32*nout,ks=ks)
                result=tf.add(result2,result)
            scale5=result
            scale5_m=self.bn_scale_relu_conv(scale3,train,nout=1,ks=ks)

            result=self.bn_scale_relu_conv(result,train,nout=32*nout,ks=7)
            # zsa：上行为白色，1*1的中间部分
        ...
        with tf.variable_scope('label_predictor'):
            result=self.bn_scale_relu_conv(result,train,nout=32*nout,conv_type='dconv',ks=7)
            scale5_u=self.bn_scale_relu_conv(result,train,nout=1,ks=ks)
            result=tf.concat(result,scale5)
            for i in range(repeat):
                if i==0:
                    result1=self.bn_scale_relu_conv(result,train,nout=16*nout,conv_type='dconv',ks=4,stride=2)
                    result=self.bn_scale_relu_conv(result,train,nout=16*nout,conv_type='dconv',ks=2,stride=2)
                else:
                    result1=self.bn_scale_relu_conv(result,train,nout=16*nout,ks=ks)
                result2=self.bn_scale_relu_conv(result1,train,nout=16*nout,ks=ks)
                result=tf.add(result2,result)
            scale4_u=self.bn_scale_relu_conv(result,train,nout=1)

            result=tf.concat(result,scale4)
            for i in range(repeat):
                if i==0:
                    result1=self.bn_scale_relu_conv(result,train,nout=8*nout,conv_type='dconv',ks=4,stride=2)
                    result=self.bn_scale_relu_conv(result,train,nout=8*nout,conv_type='dconv',ks=2,stride=2)
                else:
                    result1=self.bn_scale_relu_conv(result,train,nout=8*nout,ks=ks)
                result2=self.bn_scale_relu_conv(result1,train,nout=8*nout,ks=ks)
                result=tf.add(result2,result)
            scale3_u=self.bn_scale_relu_conv(result,train,nout=1)

            result=tf.concat(result,scale3)
            for i in range(repeat):
                if i==0:
                    result1=self.bn_scale_relu_conv(result,train,nout=4*nout,conv_type='dconv',ks=4,stride=2)
                    result=self.bn_scale_relu_conv(result,train,nout=4*nout,conv_type='dconv',ks=2,stride=2)
                else:
                    result1=self.bn_scale_relu_conv(result,train,nout=4*nout,ks=ks)
                result2=self.bn_scale_relu_conv(result1,train,nout=4*nout,ks=ks)
                result=tf.add(result2,result)
            scale2_u=self.bn_scale_relu_conv(result,train,nout=1)

            result=tf.concat(result,scale2)
            for i in range(repeat):
                if i==0:
                    result1=self.bn_scale_relu_conv(result,train,nout=2*nout,conv_type='dconv',ks=4,stride=2)
                    result=self.bn_scale_relu_conv(result,train,nout=2*nout,conv_type='dconv',ks=2,stride=2)
                else:
                    result1=self.bn_scale_relu_conv(result,train,nout=2*nout,ks=ks)
                result2=self.bn_scale_relu_conv(result1,train,nout=2*nout,ks=ks)
                result=tf.add(result2,result)
            scale1_u=self.bn_scale_relu_conv(result,train,nout=1)

            result=tf.concat(result,scale1)
            for i in range(repeat):
                if i==0:
                    result1=self.bn_scale_relu_conv(result,train,nout=nout,conv_type='dconv',ks=4,stride=2)
                    result=self.bn_scale_relu_conv(result,train,nout=nout,conv_type='dconv',ks=2,stride=2)
                else:
                    result1=self.bn_scale_relu_conv(result,train,nout=nout,ks=ks)
                result2=self.bn_scale_relu_conv(result1,train,nout=nout,ks=ks)
                result=tf.add(result2,result)
            scale0_u=self.bn_scale_relu_conv(result,train,nout=1)
            ...
            #先只看saliency部分
            metric=self.conv_bn_scale_relu(result,train,nout=16)
        ...
        with tf.variable_scope('matric_layer'):
            ...
            """
            # foreground
            num_foreground_example = np.sum(self.task_labels.reshape(self.num_cases, self.height * self.width), axis=1)
            num_foreground_example[np.where(num_foreground_example == 0)] = 1

            tmp_foreground_example = metric * \
                                     np.tile(self.task_labels, (1, self.channels, 1, 1))  # what?????
            tmp_foreground_example = tmp_foreground_example.reshape(self.num_cases, self.channels,
                                                                    self.height * self.width)
            tmp_foreground_example = tmp_foreground_example.sum(axis=2) * 1. / num_foreground_example

            # background
            num_background_example = np.sum(1 - self.task_labels.reshape(self.num_cases, self.height * self.width),
                                            axis=1)
            num_background_example[np.where(num_foreground_example == 0)] = 1

            tmp_background_example = metric * \
                                     np.tile(1 - self.task_labels, (1, self.channels, 1, 1))  # what?????
            tmp_background_example = tmp_foreground_example.reshape(self.num_cases, self.channels,
                                                                    self.height * self.width)
            tmp_background_example = tmp_foreground_example.sum(axis=2) * 1. / num_foreground_example

            # fore_back_loss
            foreground_loss = ((metric - tmp_foreground_example) ** 2 - (
                    metric - tmp_background_example) ** 2) * \
                              np.tile(self.task_labels, (1, 1, 1, self.channels))

            foreground_loss = foreground_loss.sum(axis=3)

            background_loss = ((metric - tmp_background_example) ** 2 - (
                    metric - tmp_foreground_example) ** 2) * \
                              np.tile(1 - self.task_labels, (1, 1, 1, self.channels))
        ...
        if train:
            ...
            ##############Metric loss##############
            #####################################
            self.metric_loss=total_loss.mean()
